...
Bowling::Bowling(const string& s) {
  ...
  tokens.push_back(s0);
  if (tokens.size() > 11) tokens.erase(tokens.begin() + 10);
}

vector<string> Bowling::getTokens() const { return tokens; }
...
int Bowling::countBonuses(vector<string> tokens)
{
	int points=0;
	for(int i=10;i<(int)tokens.size();i++ )
	{
